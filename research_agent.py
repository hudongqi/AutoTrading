"""
Research & Event Filter Agent for High Volatility Pool
研究整理与事件过滤 Agent - 高波动流动性池
"""
...
class EventFilterAgent:
    ...
    def evaluate_macro(self, now_utc: datetime) -> Tuple[MacroState, List[str]]:
        ...
        if macro.get("reduce_risk", False):
            notes.append(f"外部信号: REDUCE_RISK - {macro.get('reason', '不确定性升高')}")
    ...
    def evaluate_symbol(self, symbol_raw: str, now_utc: datetime) -> Tuple[DecisionLabel, List[str]]:
        data = self._load_signal_file()
        ...
        if symbol_events.get("block", False):
            ...
            return DecisionLabel.BLOCK, notes
        
        if symbol_events.get("reduce_risk", False):
            reason = symbol_events.get("reason", '币种不确定性升高')
            notes.append(f"外部信号 REDUCE_RISK: {reason}")
            return DecisionLabel.REDUCE_RISK, notes
        
        notes.append("无特定事件风险")
        return DecisionLabel.ALLOW, notes
...
class ScoringAgent:
    ...
    def apply_event_filters(self, df: pd.DataFrame, event_results: Dict[str, Tuple[DecisionLabel, List[str]]]) -> pd.DataFrame:
        for symbol_raw in POOL_SYMBOLS_RAW:
            decision, _ = event_results.get(symbol_raw, (DecisionLabel.ALLOW, []))
    ...
    def generate_markdown_summary(self, report: ResearchReport, date_str: str) -> str:
        ...
        for i, sym in enumerate(report.symbols, 1):
            lines.append(
                ...
                f"{sym.trend_4h} | {sym.atr_pct:.4f} | {sym.continuation_score:.2f} | {sym.fake_breakout_risk:.2f} |"
            )
...
class HighVolPoolResearchAgent:
    """主控制器 - 研究整理与事件过滤 Agent"""
    
    def __init__(self):
        self.data_agent = ResearchDataAgent()
    ...
    def run_daily_research(self) -> ResearchReport:
        ...
        event_results = {}
        for sym_raw in POOL_SYMBOLS_RAW:
            decision, notes = self.event_agent.evaluate_symbol(sym_raw, now)
            event_results[sym_raw] = (decision, notes)
        ...
        for _, row in score_df.iterrows():
            sym_raw = row["symbol"]
            decision, event_notes = event_results.get(sym_raw, (DecisionLabel.ALLOW, []))
            ...
            sym_research = SymbolResearch(
                ...
                has_major_event=decision != DecisionLabel.ALLOW,
